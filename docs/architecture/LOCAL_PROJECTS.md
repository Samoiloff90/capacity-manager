...
Дата: 2026-09-23. Статус: подход СОГЛАСОВАН PO в DEC-016; первая изолированная реализация проверена.
Основание: согласованный ../product/MVP.md и ../system/MODEL.md.
Расчётное ядро, адаптер снимков и native-хранилище реализованы отдельно от действующего UI.
Связка native pool → SQL-плагин проверена через IPC в MockRuntime с реальной SQLite.
Подключение production-запуска, WebView и пригодность доставки остаются отдельными проверками.
...
Основную реализацию UI не начинать до результатов проверок хранилища и доставки.
...
в изоляции; production-интеграция и поддержка конкретных ОС остаются условиями следующего этапа.
