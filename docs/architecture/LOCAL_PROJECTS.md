...
## Выгрузка отчёта (DEC-024)

Отчёт строится во фронтенде из **сохранённого** снимка: модель в `src/export/quarter-report.ts`,
XLSX в `src/export/xlsx.ts`. Файл пишет отдельный узкий Rust-модуль
`src-tauri/src/report_export.rs`, вне `project_store`: это не жизненный цикл БД. Команда
`report_save_xlsx`:
- принимает имя по умолчанию и готовые байты (не больше 2 МиБ, сигнатура ZIP);
- сама показывает системное окно «Сохранить как»;
- пишет через временный файл в той же папке и замену.

Путь выбирает пользователь в окне ОС, поэтому фронтенд не получает `fs:*` и
`dialog:allow-save`.

С этого этапа `build.rs` объявляет app manifest (`APP_COMMANDS`). Команды приложения,
включая `project_create|open|close`, проходят ACL только с явным `allow-*` в
`capabilities/default.json`; `require_main` остаётся вторым рубежом. Новая команда
добавляется в трёх местах: `generate_handler!` в `configure()`, `APP_COMMANDS` и capability.
Сгенерированные файлы `src-tauri/permissions/autogenerated/*.toml` хранятся в репозитории.
Устаревший файл удалённой команды нужно удалить вручную.

Библиотека `write-excel-file` сжимает книгу асинхронным `zip()` из fflate, а тот для
крупных частей создаёт `blob:` Web Worker, запрещённый CSP. Поэтому голый импорт `fflate`
через alias Vite/Vitest заменён синхронным шимом. CSP не ослаблялась. Результаты и
границы проверки — [../audit/REPORT_EXPORT.md](../audit/REPORT_EXPORT.md).
